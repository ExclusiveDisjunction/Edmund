...
class SubCategory : Identifiable {
    init(_ name: String, parent: Category, col_id: Int64 = 0) {
        self.name = name
        self.parent = parent
        self.col_id = col_id
    }
    ...
    static let table = Table("sub_categories")
    static let name_col = SQLite.Expression<String>("name")
    ...
    static func createTable(db: Connection) throws {
        try db.run(table.create(ifNotExists: true) {
            ...
            $0.foreignKey(parent_col, references: Category.table, Category.name_col, update: .cascade, delete: .cascade)
        })
    }
}
