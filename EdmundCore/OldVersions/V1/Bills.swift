...
extension EdmundModelsV1 {
    ...
    public final class Bill : Identifiable {
        ...
        public private(set) var _period: TimePeriods.RawValue;
    }
    
    /// An instance used to keep the order of utility data points.
    @Model
    public class UtilityDatapoint : Identifiable {
        public init(_ amount: Decimal, index: Int, parent: EdmundModelsV1.Utility?) {
            self.id = index
            self.parent = parent
            self.amount = amount
        }
    ...
    public final class Utility : Identifiable {
        ...
        public init(_ name: String, amounts: [Decimal], company: String, location: String?, start: Date, end: Date?, period: TimePeriods) {
            self.name = name
            self.startDate = start
            self.endDate = end
        ...
        public private(set) var _points: [EdmundModelsV1.UtilityDatapoint]? = nil;
        
        /// The period as a raw value
        public private(set) var rawPeriod: Int = 0;
    }
}
